...
public class DapAnController : MonoBehaviour {
	...
	void Reply(int k)
	{
		...
		AlGameController.instance.setLaiVanSam("suyluan");
		AlGameController.instance.currentState = AlGameController.State.Reply;
		AlGameController.instance.selectCase = k;
		doSetEnabal(false);

		if (AlGameController.instance.level <= 5) {
	...
	IEnumerator WaitTimeDuaRa(float time)
	{
		...
		if (chon == 0)
		{
			AlSoundController.Instance.PlayDuaRa2();
		}
		else
		{
			AlSoundController.Instance.PlayDuaRa1();
		}
		...
		else if (AlGameController.instance.level > 5 && AlGameController.instance.level <= 10)
		{
	...
	IEnumerator WaitTimeXuLy(float time)
	{
		...
		AlGameController.instance.doXuLy();
		setSprite();
	...
	void setSprite()
	{
		if (AlGameController.instance.selectCase != AlGameController.instance.truecase)
		{

			if (AlGameController.instance.selectCase == 1)
			{
				spriteA.SetSprite("selectfalse");
			}

			if (AlGameController.instance.selectCase == 2)
			{
				spriteB.SetSprite("selectfalse");
			}

			if (AlGameController.instance.selectCase == 3)
			{
				spriteC.SetSprite("selectfalse");
			}

			if (AlGameController.instance.selectCase == 4)
			{
		...
		if (AlGameController.instance.truecase == 1)
		{
			spriteA.SetSprite("selecttrue");
		}

		if (AlGameController.instance.truecase == 2)
		{
			spriteB.SetSprite("selecttrue");
		}

		if (AlGameController.instance.truecase == 3)
		{
			spriteC.SetSprite("selecttrue");
		}

		if (AlGameController.instance.truecase == 4)
		{
	...
	void btnA_Click()
	{
		try
		{
			...
			if (AlGameController.instance.currentState == AlGameController.State.Question)
			{
				...
				AlSoundController.Instance.PlayChonA();
				Reply(1);
			}
	...
	void btnB_Click()
	{
		try
		{
			if (AlGameController.instance.currentState == AlGameController.State.Question)
			{
				...
				AlSoundController.Instance.PlayChonB();
				Reply(2);
			}
		}
		catch (System.Exception)
	...
	void btnC_Click()
	{
		try{
			if (AlGameController.instance.currentState == AlGameController.State.Question)
			{
				...
				AlSoundController.Instance.PlayChonC();
				Reply(3);
			}
		}
		catch (System.Exception)
	...
	void btnD_Click()
	{
		try
		{
			if (AlGameController.instance.currentState == AlGameController.State.Question)
			{
				...
				AlSoundController.Instance.PlayChonD();
				Reply(4);
			}
		}
		catch (System.Exception)
